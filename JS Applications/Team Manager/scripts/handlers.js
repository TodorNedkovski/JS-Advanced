...
export async function catalogeHandler(){
    ...
    this.teams = Object.entries(await fireBaseRequestFactory(`https://teammanager-3ffdb.firebaseio.com/`, 'teams', token).getAll().then(x => x || {})).map(([id, value]) => ({ _id: id, ...value }))
...
export async function catalogueDetailsHandler(){
    ...
    const teams = await fireBaseRequestFactory(`https://teammanager-3ffdb.firebaseio.com/`, 'teams', token).getAll()
...
export async function joinTeamHandler(){
    ...
    await fireBaseRequestFactory(`https://teammanager-3ffdb.firebaseio.com/`, 'teams', token).patchEntity(team, teamId)
...
export async function editTeamHandler(){
    ...
    const members = await fireBaseRequestFactory(`https://teammanager-3ffdb.firebaseio.com/`, 'teams', token).getById(teamId).members
